...
CREATE TABLE `ball_admin` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8 COMMENT='后端管理账号';
...
CREATE TABLE `ball_balance_change` (
  ...
  `balance_change_type` tinyint(4) DEFAULT '0' COMMENT '1充值 2提现 3投注 4赢 5佣金 6人工 ',
...
CREATE TABLE `ball_logger` (
  ...
  `admin_id` bigint(20) DEFAULT '0' COMMENT '管理员id  默认为0',
  `player_id` bigint(20) DEFAULT '0' COMMENT '玩家日志 默认为 0',
  `content` varchar(255) DEFAULT '' COMMENT '日志 内容 ',
...
CREATE TABLE `ball_player` (
  ...
  `the_last_ip` tinyblob NOT NULL COMMENT '上一次登录的ip',
